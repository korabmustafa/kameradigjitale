...
const products: Prisma.ProductCreateInput[] = [
  {
    ...
    category: ProductCategory.DIGITAL_CAMERAS,
    ...
    specs: {
      create: [
        { label: 'Brand', value: 'Nikon', position: 0 },
        { label: 'Sensor', value: 'Full frame', position: 1 }
      ]
    }
  },
  {
    ...
    category: ProductCategory.FILM_CAMERAS,
    price: new Prisma.Decimal('390.00'),
    stock: 4,
    ...
    specs: {
      create: [
        { label: 'Brand', value: 'Canon', position: 0 },
        { label: 'Format', value: '35mm film', position: 1 }
      ]
    }
  },
  {
    ...
    specs: {
      create: [
        { label: 'Brand', value: 'Sony', position: 0 },
        { label: 'Mount', value: 'Sony E', position: 1 }
      ]
    }
  }
];
...
async function main() {
  for (const product of products) {
    await prisma.product.upsert({
      ...
      update: {
        ...product,
        specs: { deleteMany: {}, create: product.specs?.create ?? [] }
      }
    });
  }
  ...
  await prisma.adminUser.upsert({
    ...
    create: {
      name: 'Rina Akter',
      email: 'rina@kameradigjitale.local',
      role: UserRole.ADMIN,
      active: true
    },
  ...
  const menu = [
    ['New', '/new', null],
    ['Film Cameras', '/catalog/film-cameras', ProductCategory.FILM_CAMERAS],
    ['Digital Cameras', '/catalog/digital-cameras', ProductCategory.DIGITAL_CAMERAS],
    ['Lenses', '/catalog/lenses', ProductCategory.LENSES],
    ['Film', '/catalog/film', ProductCategory.FILM],
    ['Accessories', '/catalog/accessories', ProductCategory.ACCESSORIES],
    ['Supplies', '/catalog/supplies', ProductCategory.SUPPLIES],
    ['Brands', '/brands', null],
  ...
  await Promise.all(
    menu.map(([label, path, category], idx) =>
  ...
  const navigationSubcategories = [
    [
      ProductCategory.DIGITAL_CAMERAS,
      'mirrorless',
      'Mirrorless',
