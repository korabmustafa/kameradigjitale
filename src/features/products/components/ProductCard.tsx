  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-playful">
      ...
      <div className="space-y-3 p-4">
        <span className="inline-block rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{product.category}</span>
        <Link to={`/products/${product.productCode}`}>
          <h3 className="text-lg font-bold text-slate-900 transition hover:text-ink">{product.name}</h3>
        </Link>
